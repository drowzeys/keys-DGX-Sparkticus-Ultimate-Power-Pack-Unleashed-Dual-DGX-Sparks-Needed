...
# Resolve assets next to this script (repo / package), then ~/comfy
...
def kf_prompt(plan, kf):
    """MiniMax H3 official T2VA/I2VA keyframe prompt (see h3_prompt_guide.py)."""
    ...
    return PG.kf_t2va_or_i2va(
        ...
        tone=plan["tone"],
        kf_prompt=kf["prompt"],
        audio_bed=plan.get("audio_bed", "Natural outdoor ambience."),
        has_first_frame=False,
    )


def span_prompt(plan, span):
    """MiniMax H3 official FL2VA path prompt between keyframe pictures."""
    import h3_prompt_guide as PG
    L = plan.get("span_len", 124)
    return PG.fl2va_span(
        style=plan["style"],
        lighting=plan["lighting"],
        tone=plan["tone"],
        motion=span["motion"],
        audio_bed=plan.get("audio_bed", ""),
        extra_audio=span.get("audio", ""),
        say=span.get("say", "") or "",
        length_frames=L,
        fps=24.0,
        identity_note=span.get("identity_note", "Keep first-person continuity and scale consistent with Picture 1."),
    )
